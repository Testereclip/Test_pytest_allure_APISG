...
def cargar_casos():
    ...
    with archivo_json.open(encoding="utf-8") as f:
        ...
        for entrada in data:
            ...
            for atributo in [
                "tipodireccion", "codpartido", "tipocatastro",
                "parcelainf", "manzanainf", "sectorinf", "barrioinf"
            ]:
                if atributo in entrada:
                    caso[atributo] = entrada[atributo]
...
# Validaciones opcionales para atributos adicionales
        ...
        for atributo in atributos_opcionales:
            if caso.get(atributo):
                assert data.get(atributo) == caso[atributo], (
                    f"[({x}, {y})] {atributo} esperado: {caso[atributo]}, obtenido: {data.get(atributo)}"
                )
